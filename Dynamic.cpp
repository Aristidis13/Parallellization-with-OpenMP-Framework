...
int main(int argc, char *argv[])
{
	...
	if (argc != 5)
	{
		cout << "USAGE: ./RDP <input file name> <epsilon> <print_results> <thread_number>" << endl;
		exit(1);
	}
...
/* Start main calculation. */
	...
	#pragma omp parallel num_threads(numberOfThreads) 
	{
	    ...
	    gettimeofday(&s, NULL);
	    int mem_chunk=1;//Defines how many lines will be processed by one thread at a time
	    #pragma omp for schedule(dynamic,mem_chunk) nowait
	    for(int k=0;k<numberOfLinesInFile;k++)
	    {
		    RamerDouglasPeucker(AllPolylines[k],epsilon,SimplifiedAllPolylines[k]);
		}
		gettimeofday(&e, NULL);
	    total_time = ((e.tv_sec - s.tv_sec) * 1000000.0 + (e.tv_usec - s.tv_usec))/ 1000000.0;
...
/* If requested, print out the initial and the simplified polylines. */
	...
	cout << "The number of lines in file are " <<numberOfLinesInFile << endl
	     <<"The vector has size "<< SimplifiedAllPolylines.size() << endl;
